...
export const ProductsList = ({ projects }) => {
	useEffect(() => {
		Aos.init({
			offset: 150,
			mirror: false,
	...
	return (
		<StyledPortfolioList>
			{projects.map((el, index) => {
				const { slug, longImage, pageTitle, background } = el.attributes;
						...
						>
							<ProjectContainer background={background}>
								<ImageContainer>
									{longImage && (
										<img
											src={`${process.env.GATSBY_API_DATA_URL}${longImage.data.attributes.url}`}
											alt={longImage.data.attributes.alternativeText}
										/>
									)}
								</ImageContainer>
							</ProjectContainer>
