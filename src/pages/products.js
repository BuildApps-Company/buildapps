...
import { ButtonsListProducts } from '../components/PortfolioPage/ButtonsListProducts';
import { ProductsList } from '../components/PortfolioPage/ProductsList';
import { sendRequestToAPI } from '../api/use-cases';
...
export default function PortfolioPage({
	...
	const [filteredElements, setFilteredElements] = useState(useCases);
		...
		const getData = async () => {
			try {
				const { data } = await sendRequestToAPI(
					`query MyQuery ($filter: UseCaseFiltersInput, $locale: I18NLocaleCode ) {
							useCases(filters: $filter, locale: $locale) {
								data {
									attributes {
										slug
										background
				...
				setFilteredElements(data.data.useCases);
			} catch (err) {
				return err;
			}
		};
		category !== filterResetBtn ? getData() : setFilteredElements(useCases);
	}, [selectedCategories]);
	...
	return (
		<Page pageName="Portfolio">
			...
			<ContainerWrap>
				...
				<ProductsList projects={filteredElements.data} />
			</ContainerWrap>
